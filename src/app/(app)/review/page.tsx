    ...
    <div className="mx-auto max-w-3xl space-y-6">
        ...
        >
          <ul className="space-y-1.5">
            {goalsSlipping.map(({ goal, openCount }) => (
              <li key={goal.id} className="flex items-center justify-between gap-3 rounded-lg bg-surface-2 px-3 py-2">
                <div className="min-w-0">
                  <p className="truncate text-sm">{goal.title}</p>
                  <p className="text-xs text-text-secondary">{openCount} open task{openCount !== 1 ? "s" : ""}, none scheduled</p>
                </div>
      ...
      >
        {staleBacklog.length === 0 ? (
          <p className="text-sm text-text-disabled">No stale tasks.</p>
        ) : (
          <ul className="space-y-1.5">
            {staleBacklog.map((t) => (
              <li key={t.id} className="flex items-center justify-between gap-3 rounded-lg bg-surface-2 px-3 py-2">
      ...
      >
        <div className="space-y-3">
          {activeGoals.length === 0 && <p className="text-sm text-text-disabled">Create a goal first — it gives this review something to work toward.</p>}
          {activeGoals.map((goal) => {
            const candidates = candidatesByGoal.get(goal.id) ?? []
            if (!candidates.length) return null
            const checked = big3[goal.id] ?? []
            return (
              <div key={goal.id} className="rounded-lg bg-surface-2 p-3">
                <p className="mb-2 text-xs font-medium uppercase tracking-wider text-text-secondary">{goal.title}</p>
