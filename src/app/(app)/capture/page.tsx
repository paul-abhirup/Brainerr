  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div className="text-center space-y-2">
        <Badge className="bg-primary/15 text-primary border-primary/30 font-bold uppercase tracking-wider text-xs px-3 py-1">
      ...
      {/* Main Input Box */}
      <Card className="border-2 border-primary/40 shadow-md p-6 relative group">
        <form onSubmit={handleDumpSubmit} className="space-y-4">
          <div className="relative flex items-center">
            <Input
              ...
              className="h-16 text-lg font-medium pl-5 pr-28 rounded-xl bg-secondary/90 border-border focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all"
            />
            ...
            <div className="absolute right-3 flex items-center gap-2">
              ...
              <Button
                ...
                size="icon"
                className="h-10 w-10 rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 shadow-md cursor-pointer"
              >
          ...
          <div className="flex items-center justify-between text-xs text-disabled px-2">
            <span>Press <kbd className="px-1.5 py-0.5 rounded bg-secondary border border-border font-mono text-xs">Enter ↵</kbd> to save & keep typing</span>
            {dumpedCount > 0 && (
      ...
      <Card className="border-border shadow-lg p-4 space-y-4">
        ...
        <div className="space-y-2">
          {recentDumps.length === 0 ? (
            <div className="text-center py-8 text-xs text-disabled">
            ...
            recentDumps.map((t) => (
              <div
                key={t.id}
                className="p-3 rounded-xl border border-border bg-card/90 hover:border-primary/50 transition-all flex items-center justify-between text-sm"
              >
                <span className="font-medium truncate text-foreground flex-1 mr-4">{t.title}</span>
                <Badge variant="outline" className="text-xs uppercase text-disabled shrink-0">
                  Inbox
                </Badge>
              </div>
            ))
